...
<section class="content">
    <div class="container-fluid">
        <div class="text-right mb-3">
            <a href="{{ route('admin.passenger.create') }}" class="btn bg-gradient-primary px-5">Nova passageiro</a>
        </div>
                    ...
                    <tbody>
                        @foreach ($passengers as $passenger)
                            <tr>
                                ...
                                <td>
                                    {{-- <a href="{{ route('admin.passenger.show', $passenger->cd_psgr) }}" class="btn btn-success btn-sm" data-toggle="tooltip" title="Visualizar passageiro">
                                        <i class="fas fa-eye"></i>
                                    </a> --}}
                                    <a href="{{ route('admin.passenger.edit', $passenger->cd_psgr) }}" class="btn btn-info btn-sm" data-toggle="tooltip" title="Editar passageiro">
                                    ...
                                    <form method="post" action="{{ route('api.passenger.destroy', $passenger->cd_psgr) }}" id="delete_{{ $passenger->cd_psgr }}" class="form-delete-passenger">
                                        {{ csrf_field() }}
                                        {{ method_field('DELETE') }}
                                    </form>
                                </td>
    ...
    <script>
        $(document).ready(function () {
            ...
            const ps_delete = new PSDelete({
                selector: '.form-delete-passenger',
                title: 'Tem certeza de que deseja deletar este passageiro?',
