...
<section class="content">
    <div class="container-fluid">
        <div class="text-right mb-3">
            <a href="{{ route('admin.airport.create') }}" class="btn bg-gradient-primary px-5">Novo aeroporto</a>
        </div>
                    ...
                    <tbody>
                        @foreach ($airports as $airport)
                            <tr>
                                ...
                                <td>
                                    {{-- <a href="{{ route('admin.airport.show', $airport->cd_arpt) }}" class="btn btn-success btn-sm" data-toggle="tooltip" title="Visualizar aeroporto">
                                        <i class="fas fa-eye"></i>
                                    </a> --}}
                                    <a href="{{ route('admin.airport.edit', $airport->cd_arpt) }}" class="btn btn-info btn-sm" data-toggle="tooltip" title="Editar aeroporto">
                                    ...
                                    <form method="post" action="{{ route('api.airport.destroy', $airport->cd_arpt) }}" id="delete_{{ $airport->cd_arpt }}" class="form-delete-airport">
                                        {{ csrf_field() }}
                                        {{ method_field('DELETE') }}
                                    </form>
                                </td>
    ...
    <script>
        $(document).ready(function () {
            ...
            const ps_delete = new PSDelete({
                selector: '.form-delete-airport',
                title: 'Tem certeza de que deseja deletar este aeroporto?',
