...
<section class="content">
    <div class="container-fluid">
        <div class="text-right mb-3">
            <a href="{{ route('admin.country.create') }}" class="btn bg-gradient-primary px-5">Novo país</a>
        </div>
                    ...
                    <tbody>
                        @foreach ($countries as $country)
                            <tr>
                                ...
                                <td>
                                    {{-- <a href="{{ route('admin.country.show', $country->cd_pais) }}" class="btn btn-success btn-sm" data-toggle="tooltip" title="Visualizar país">
                                        <i class="fas fa-eye"></i>
                                    </a> --}}
                                    <a href="{{ route('admin.country.edit', $country->cd_pais) }}" class="btn btn-info btn-sm" data-toggle="tooltip" title="Editar país">
                                    ...
                                    <form method="post" action="{{ route('api.country.destroy', $country->cd_pais) }}" id="delete_{{ $country->cd_pais }}" class="form-delete-country">
                                        {{ csrf_field() }}
                                        {{ method_field('DELETE') }}
                                    </form>
                                </td>
    ...
    <script>
        $(document).ready(function () {
            ...
            const ps_delete = new PSDelete({
                selector: '.form-delete-country',
                title: 'Tem certeza de que deseja deletar este país?',
